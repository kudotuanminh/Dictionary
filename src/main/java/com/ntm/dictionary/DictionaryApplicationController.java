    ...
    private void addWordDialog() throws SQLException, ClassNotFoundException {
        ...
        if (this.getIndex(thisWordTarget) == -1) {
            ...
            if (result.get() == buttonTypeSave) {
                ...
                thisWordTarget = thisWordTarget.replace("'", "''");
                thisWordExplain = thisWordExplain.replace("'", "''");
        ...
        if (!wordViewList.getItems().isEmpty()) {
            ...
            if (this.getIndex(newWordTarget) == -1) {
                ...
                if (result.get() == buttonTypeSave) {
                    ...
                    Statement statement = this.newStatement();
                    statement.executeUpdate(
                            "UPDATE av SET word='" + newWordTarget
                                    + "', description='" + newWordExplain
                                    + "' WHERE word='" + thisWordTarget + "'");
                ...
                Alert alert = new Alert(Alert.AlertType.ERROR);
                alert.setContentText("\"" + newWordTarget
                        + "\" already exist. Please choose a different wordTarget.");
                alert.show();
            }
        } else {
            Alert alert = new Alert(Alert.AlertType.ERROR);
